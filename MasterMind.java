...
public class MasterMind {
    ...
    public static int indexOf(char[] arr, char ch) {
        ...
        for (i = 0; i < arr.length; ++i) { //loops until arr matches ch
            if (arr[i] == ch) {
                status = true;
                break;
            }
        }
        if (i != 0 && status) {//only returns valid values
            return i; 
        }
        else {
            return -1;
    ...
    public static char[] generateHiddenCode(Random rand, int numPositions, char[] symbols) {
        char[] result = new char[numPositions];
        int i;
        for (i = 0; i < numPositions; ++i) {
            System.out.println(rand.nextInt(4));
            result [i] = symbols[rand.nextInt(4)];
        }
